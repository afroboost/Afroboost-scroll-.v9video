    ...
    if reservation.userEmail:
        try:
            if RESEND_AVAILABLE and RESEND_API_KEY:
                ...
                qr_url = f"https://api.qrserver.com/v1/create-qr-code/?size=150x150&data={qr_data}&format=png"
    ...
    try:
        ...
        if event.type == 'checkout.session.completed':
            ...
            if RESEND_AVAILABLE and RESEND_API_KEY and customer_email:
                ...
                html = f"""<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0a0a0a;"><div style="background:linear-gradient(135deg,#d91cd2,#8b5cf6);padding:24px;text-align:center;"><h1 style="color:white;margin:0;font-size:22px;">Bienvenue chez Afroboost</h1></div><div style="padding:24px;color:#fff;"><p style="color:#a855f7;font-size:16px;">Merci pour votre achat !</p><div style="background:rgba(147,51,234,0.15);border:1px solid rgba(147,51,234,0.3);border-radius:12px;padding:20px;margin:20px 0;text-align:center;"><p style="margin:0 0 8px;color:#888;">Votre code d'acces</p><p style="margin:0;color:#d91cd2;font-size:28px;font-weight:bold;letter-spacing:3px;">{new_code}</p><p style="margin:12px 0 0;color:#888;">{sessions_count} seances incluses</p></div><div style="text-align:center;margin:30px 0;"><p style="color:#888;margin-bottom:16px;">Votre QR Code d'acces</p><img src="{qr_url}" alt="QR Code Afroboost" width="150" height="150" style="background:white;padding:10px;border-radius:8px;display:block;margin:0 auto;"/></div><div style="text-align:center;margin:30px 0;"><a href="https://afroboosteur.com" style="display:inline-block;background:#d91cd2;color:white;padding:14px 28px;text-decoration:none;border-radius:8px;font-weight:bold;font-size:14px;">Acceder a mon espace Afroboost</a></div><p style="color:#666;font-size:12px;text-align:center;margin-top:30px;">Conservez ce mail. Presentez le QR Code ou le code a l'entree.</p></div></div>"""
                try:
